...
const EnrollmentDataCards = () => {
  ...
  const renderEnrollmentsTable = () => {
    ...
    if (enrollmentEntries.length === 0) {
      return (
        <Empty
          image={<ExclamationCircleOutlined style={{ fontSize: 40 }} />}
          description={<span>You should have enrolled students</span>}
        />
      );
    }
  ...
  return (
    <div style={{ padding: 40 }}>
      ...
      <div id="pdf-content">
        ...
        <h3 style={{ marginBottom: 20 }}>Profit From Each Course</h3>
        {loading ? (
          <Spin />
        ) : (
          <Table
        ...
        <h3 style={{ marginBottom: 20 }}>Total Revenue By Month</h3>
        {loading ? <Spin /> : renderRevenueTable()}
        <h3 style={{ marginBottom: 20, marginTop: 60 }}>
